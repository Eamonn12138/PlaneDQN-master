...
class GameState:
    ...
    def frame_step(self, input_actions):
        ...
        # 循环15次发射一个子弹
        # 检测输入正确性
        if input_actions[0] == 1 or input_actions[1]== 1 or input_actions[2]== 1 or input_actions[3]== 1 or input_actions[4]== 1 or input_actions[5]== 1:  # 检查输入正常
            if input_actions[0] == 0 and input_actions[1] == 1 and input_actions[2] == 0 and input_actions[3] == 0 and input_actions[4] == 0 and input_actions[5] == 0:
                self.player.moveLeft()
            elif input_actions[0] == 0 and input_actions[1] == 0 and input_actions[2] == 1 and input_actions[3] == 0 and input_actions[4] == 0 and input_actions[5] == 0:
                self.player.moveRight()
            elif input_actions[0] == 0 and input_actions[1] == 0 and input_actions[2] == 0 and input_actions[3] == 1 and input_actions[4] == 0 and input_actions[5] == 0:
                self.player.shoot(self.bullet_img)

                reward = reward - 0.4
            elif input_actions[0] == 0 and input_actions[1] == 0 and input_actions[2] == 0 and input_actions[3] == 0 and input_actions[4] == 1 and input_actions[5] == 0:
                self.player.shoot(self.bullet_img)

                reward = reward - 0.4
                self.player.moveLeft()

            elif input_actions[0] == 0 and input_actions[1] == 0 and input_actions[2] == 0 and input_actions[3] == 0 and input_actions[4] == 0 and input_actions[5] == 1:
                self.player.shoot(self.bullet_img)
        ...
        for enemy in self.enemies1:
            ...
            if pygame.sprite.collide_circle(enemy, self.player):
                ...
                reward =reward-6   #撞击后退出
                #reward = reward - 2
                write_score=self.score+reward

                size = os.path.getsize(PATH)
                if size==0:
                    file_jorney = open(PATH, 'a+')
                    file_jorney.write('1 —— '+str(round(write_score,2))+'\n')
                    file_jorney.close()
                else:
                    file_jorney = open(PATH, 'r')
                    file =file_jorney.readlines()
                    file_jorney.close
                    theid=int(file[-1].split(' ')[0])+1
                    file_jorney = open(PATH, 'a')
                    file_jorney.write('{} —— '.format(theid)+ str(round(write_score,2))+'\n')
                    file_jorney.close()
        ...
        if self.player.is_hit == True:
            self.score=0
        else:
            self.score+=reward
        score_text = score_font.render(str(round(self.score,2)), True, (128, 128, 128))
